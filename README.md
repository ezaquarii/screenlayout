...
Create a keybinding that execs `screenlayout.py configure pp` to
configure both screens in portrait mode.

Run `screenlayout.py configure off` to disable them.

Run ``screenlayout.py -v --dry-run configure pp` to see `xrandr`
command without running any action.
